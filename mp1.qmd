...
word <- c("woman", "women", "girl", "female", "lady", "girls", "women's", "ladies") # these words are more popular in this data set, yet do not reveal much information about the headlines
...
```{r, echo = FALSE}
headlines |>
  mutate(length = str_length(headline_no_site))|> # find character length of headlines
...
```{r, echo = FALSE}
headlines |>
  mutate(question = str_extract(headlines$headline_no_site, ".*\\?")) |> # find questions in headlines
  select(question) |>
...
headlines_no_stop |>
  ...
  slice_max(n, n = 10) |> # top ten words in the headlines
  ggplot(aes(x = fct_reorder(word, n), y = n)) + # so bars on the graph are in order of frequency
  geom_col() +
...
```{r, include = FALSE}
headlines_wordcloud <- headlines_no_stop |>  # create separate frame for the word cloud to include counts
  count(word) |>
...
```{r, include = FALSE}
bing_sentiments <- get_sentiments(lexicon = "bing") # to be joined with headlines data
```
...
headlines_no_stop |>
  ...
  group_by(sentiment) |> # words are counted by sentiment and both sentiments are included in the slice_max()
  count(word, sentiment, sort = TRUE) |>
  slice_max(n, n = 10) |> 
  ggplot(aes(x = fct_reorder(word, n), y = n, fill = sentiment)) + # so bars on the graphs are in order of frequency
  geom_col() +  
  coord_flip() + # nicer visually 
...
```{r, eval = FALSE, include = FALSE}
str_subset(headlines$headline_no_site, "beaut") # find examples of sentences that include "beauty" or "beautiful"
```
...
headlines_no_stop |>
  inner_join(bing_sentiments) |>
  filter(word != "trump", # filter out the name "trump" as it is not insightful
         sentiment == "positive") |>
...
headlines |>
  filter(str_detect(headlines$headline_no_site, " m[ea]n ")) |> # detect any headlines that include "man" or "men" separate from "wo<men>"
  unnest_tokens(output = word, input = headline_no_site) |> # make every word its own row
